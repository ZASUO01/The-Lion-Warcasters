...
bool SocketUtils::sendPacketToV4(const int sock, const Packet *pk, const size_t pkSize, sockaddr_in addr4) {
    ...
    if (const ssize_t bytes_sent = sendto(sock, pk, pkSize, 0, reinterpret_cast<sockaddr *>(&addr4), addr_size);
...
bool SocketUtils::receivePacketFromV4(const int sock, Packet *pk, sockaddr_in addr4) {
    ...
    if (const ssize_t bytes_received = recvfrom(sock, pk, pkSize, 0, reinterpret_cast<sockaddr *>(&addr4), &addrSize);
        bytes_received <= 0) {
        return false;
    }
    return true;
